...
public class PrivateLessonEndpoint {
	...
	public CollectionResponse<PrivateLesson> listPrivateLesson(
		...
		try {
			...
			Query query = mgr
					.createQuery("select from PrivateLesson as PrivateLesson");
		...
		return CollectionResponse.<PrivateLesson> builder().setItems(execute)
	...
	private boolean containsPrivateLesson(PrivateLesson privatelesson) {
		...
		try {
			PrivateLesson item = mgr.find(PrivateLesson.class,
					privatelesson.getKey());
			if (item == null) {
				contains = false;
			}
		} finally {
