...
public class SpottedPostListLoader extends AsyncTaskLoader<CollectionResponsePostSpotted> {
    ...
    public CollectionResponsePostSpotted loadInBackground() {
		...
		try {
			if(cursor!=null){
				list = endpoint.listPostSpotted().setLimit(10).setCursor(cursor).execute();
			}else {
     ...
     */
    @Override public void deliverResult(CollectionResponsePostSpotted listOfData) {
        if (isReset()) {
     ...
     */
    @Override public void onCanceled(CollectionResponsePostSpotted apps) {
